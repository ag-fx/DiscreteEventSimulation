...
class CstArrivedOnT2Event(val core: SimCore, execTime: Double) : Event(core, execTime) {
    override fun exec() {
        val customer = Customer(execTime)
//        log("$customer arrived on T2")
        core.t2Queue.add(customer)
        if (core.hasTime()) {
            val nextArrival = CstArrivedOnT2Event(core, core.currentTime + core.rArrivalToT2.nextDouble())
